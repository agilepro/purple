...
public class MemFileTester implements TestSet {
    ...
    private void testMemFiles(String caseDescription, String testCase) throws Exception {
        ...
        tr.testInt(
                caseDescription + " - Num bytes for 1000 copies",
                mf.totalBytes(),
                1000 * bytesInCase);
        tr.testInt(
                caseDescription + " - Num chars for 1000 copies",
                mf.totalChars(),
    ...
    private void testReadByte(
            String caseDescription, TestRecorder tr, MemFile mf, byte[] encodedCase)
            throws Exception {
        // read to see if the exact thing can be read out of the input stream
    ...
    private String findPosDiff(String str1, String str2) {
        ...
        for (int i = 0; i < len1; i++) {
            ...
            if (ch1 != ch2) {
                return "They differ at character "
                        + i
                        + " which is '"
                        + ch1
                        + "' and '"
