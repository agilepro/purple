...
public class Test2 implements TestSet {
    ...
    private void testRandomMatrix(
            int rowMin, int rowMax, int colMin, int colMax, List<String> sourceVals, String desc)
            throws Exception {
        ...
        for (List<String> row : matrix) {
            ...
            if (parsedRow == null) {
                throw new Exception(
                        desc
                                + "- Fatal error, the generated CSV file does not have a row "
                                + count);
            }
            testVal(
                    parsedRow.size(),
                    row.size(),
                    desc + "- output row and input row should both be " + row.size());
            ...
            for (int i = 0; i < row.size(); i++) {
                ...
                testVal(
                        ...
                        desc
                                ...
                                + ","
                                + i
                                + ") result should be the same as source");
            }
    ...
    private List<List<String>> generateMatrix(
            int rowMin, int rowMax, int colMin, int colMax, List<String> sourceVals) {
