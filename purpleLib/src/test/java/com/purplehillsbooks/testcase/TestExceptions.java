...
public class TestExceptions extends TestAbstract implements TestSet {
    ...
    public void runTests(TestRecorder newTr) throws Exception {
        ...
        compareString(
                ...
                String.format("test %s it works", "whether"));
        compareString(
                newTr,
                "testing format 2",
                "test whether it works two",
                String.format("test %s it works %s", "whether", "two"));
        String[] myBundle = new String[] {"whether", "two"};
        compareString(
                newTr,
                "testing format 3",
                "test whether it works two",
                String.format("test %s it works %s", (Object[]) myBundle));

        JSONException je = new JSONException("this is a {0} good template", "very");
        compareString(
                newTr,
                "JSONException one parameter",
                "this is a very good template",
                je.getMessage());
        je = new JSONException("this has {0} and {1} value", "first", "second");
        compareString(
                newTr,
                "JSONException two params",
                "this has first and second value",
                je.getMessage());
        je = new JSONException("three {0} values {1} to {2} see", "nogood", "inevibatly", "hardly");
        compareString(
                newTr,
                "JSONException three params",
                "three nogood values inevibatly to hardly see",
                je.getMessage());
        JSONException je2 =
                new JSONException(
                        "NESTED {0} values {1} to {2} see", je, "nogood", "inevibatly", "hardly");
        compareString(
                newTr,
                "JSONException three params NESTED",
                "NESTED nogood values inevibatly to hardly see",
                je2.getMessage());
    }
    ...
    private void callHigher(String course) throws Exception {
        if (course == null || course.length() == 0) {
            throw new Exception(
                    "top level exception thrown by exception test when course string ran out");
        }
    ...
    */
    ...
    private void testExceptionStackTrace(String course, String fileNameBase) {
        ...
        // the method callHigher is a recursive method that then throws and catches/rethrow at
        // various levels
        try {
            unique2 = 0;
            callHigher(course);
        ...
        try {
            ...
            JSONObject receivedExceptionForm =
                    JSONException.convertToJSON(testException, "Test file " + fileName);
    ...
    private void compareExceptionToFile(Exception testException, String fileName) throws Exception {
        ...
        if (!compareFile.exists()) {
            tr.markFailed(
                    fileName,
                    "testExceptionStackTrace: the test file is missing from source folder: "
                            + compareFile);
            return;
        }
        ...
        if (s1.equals(s2)) {
            ...
            tr.markFailed(
                    fileName,
                    "testExceptionStackTrace: these files do not match ("
                            + resultFile
                            + ") and ("
                            + compareFile
                            + ")");
