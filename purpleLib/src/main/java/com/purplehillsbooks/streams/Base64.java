...
/**
 * You always need a Base64 encoder and decoder, but for some reason Java does not introduce it
 * until version 1.8, and even so there are many incompatible libraries out there. This one does the
 * decoding without any dependence upon other classes, nor on a complex bloated object class
 * hierarchy that is the grandiose scheme of some programmer to impress others with. This
 * implementation is straightforward and simple to use.
 *
 * <p>To encode: simply call "encode" with the array of bytes that you want encoded, and an encoded
 * string is returned.
 *
 * <p>To decode: pass the string into "decode" and a byte array is returned.
 *
 * <p>That is all you need, and I am constantly amazed at how programmers can make something as
 * simple as this so complicated that you have to read a manual to be able to use it.
 */
public class Base64 {

    /**
     * Encodes by byte array. When byte arrays specified for the argument are null or 0 bytes, the
     * character string of the return value becomes empty string.
     *
     * @param bytes a Byte array to be encoded @return String Encoded Character string
     */
    public static String encode(byte[] bytes) {
        ...
        ByteArrayOutputStream bout = new ByteArrayOutputStream(bytes.length * 150 / 100);
    ...
    /**
     * Deciphers the passed encoded string. And it return the result in a byte array. If the
     * specified character string can not be deciphered Exception is thrown.
     *
     * @param str Character string to be deciphered @return byte[] Byte array containing the
     *     deciphered bytes @exception Exception If failed during the decipherment.
     */
     ...
     */
    private static byte[] decode3byte(byte[] bb, int offset, int max) throws IOException {
        int num = 0x00000000;
        int len = 0;
        for (int i = 0; i < 4; i++) {
            if (offset + i >= max || bb[offset + i] == '=') {
                if (i < 2) {
                    throw new IOException("BASE64Decoder: Incomplete BASE64 character");
                } else {
                    break;
                }
            }
    ...
    private static byte unmap(int cc) throws IOException {
        if (cc >= 'A' && cc <= 'Z') {
            ...
            throw new IOException("BASE64Decoder: Illegal character:= " + (char) cc);
        }
    }
}
