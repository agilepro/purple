...
/**
 * Given an InputStream to read a raw NNTP message body, this will decode the UUEncoding and give
 * you the decoded result of what the body was holding. (generally the result is a binary file
 * stream)
 *
 * <p>The format of the file is: begin {mode} {file}{newline} {length byte}{formatted
 * bytes}{newline} {length byte}{formatted bytes}{newline} {length byte}{formatted bytes}{newline}
 * {length byte}{formatted bytes}{newline} `{newline} end{newline}
 *
 * <p>Four formatted bytes are consumed output three result bytes lines before the begin and after
 * the end are ignored. Exceptions are thrown if the begin is not found, if the end is not found, or
 * if the lines between are nonsense.
 *
 * <p>"No strings or characters are used in the processing of the bytes"
 *
 * <p>Author: Keith D Swenson, August 2012
 */
public class UUDecoderStream extends InputStream {
    ...
    /**
     * Read the next decoded byte from this input stream. The byte is returned as an <code>int
     ...
     *
     * @return next byte of data, or <code>-1</code> if the end of stream is reached.
    ...
    /**
     * UUencoded streams start off with the line: "begin <mode> <filename>" Search for this prefix
     * and gobble it up.
     */
    ...
    private boolean decode() throws IOException {
        ...
        if (inSize < need + 1) {
            throw new IOException(
                    "Short buffer error: only have a line of '"
                            + inSize
                            + "' chars and we think we need '"
                            + need
                            + "'");
        }
    ...
    /**
     * tells you if the input buffer starts with a particular string pattern (assuming no encoding
     * ... ascii only)
     */
    ...
    /**
     * Look at a range of the input buffer, and return that as an integer value ... no exceptions If
     * it is not a number as all, it simply returns zero.
     */
    ...
    /**
     * Skip any leftover CR or LF bytes, and read the next line from input and store in the
     * inBuffer. Line is terminated by the next CR. LF characters are ignored and thrown away in all
     * cases. Blank lines are ignored (skipped).
     *
     * <p>maxLineLen limits the length of a line. If a CR is not found before the limit is reached,
     * then then characters up the limit are returned. Reading will resume at that point when the
     * next readOneLine is called. This effectively breaks very long lines into multiple line
     * results of maxLineLength.
     *
     * <p>Resulting bytes are placed into inBuffer and inSize tells you how many have been read.
     *
     * <p>inputDone flag is set when the source stream is empty, but keep in mind the current
     * inBuffer may still have data in it.
     */
