...
/**
 * WebRequest provides a lot of convenience functions for handling web requests especially when they
 * are JSON objects being sent back and forth. The posted JSON object is provided in JSONObject
 * form, and WebRequest can easily send JSONObjects as well.
 *
 * <p>Most important thing to learn is the path parser. Given a path like this
 *
 * <p>http://server/app/p1/p2/p3/obj.json
 *
 * <p>This will be parsed into separate tokens, starting with p1 (assuming this is the first path
 * element beyond where the servlet is mounted). Each token can be consumed one at a time with
 * #consumePathToken() This is intended for a recursive descent style parser which has a method for
 * p1, which consumes p2 and calls the method for p2 and so on. This parser properly handles the
 * accidental double slash character and it decodes the path elements.
 */
public class WebRequest {
     ...
     * Response object which only allows the writer to be gotten once. Later attempts fail and so
     * this constructor does not attempt to do this.
     *
     * <p>By getting the writer, we do not have access to teh outputstream.
     *
     * <p>There are a couple methods that require the output stream and will not work when
     * constructed from this constructor.
     */
    public WebRequest(HttpServletRequest _req, HttpServletResponse _resp, Writer aw)
            throws Exception {
        request = _req;
        response = _resp;
    ...
    /**
     * This factory method is used within a JSP file where the Writer has already been grabbed from
     * the Response object which only allows the writer to be gotten once. Later attempts fail and
     * so this factory method does not attempt to do this.
     *
     * <p>By getting the writer, we do not have access to teh outputstream.
     *
     * <p>There are a couple methods that require the output stream and will not work when
     * constructed from this constructor.
     */
    public static WebRequest findOrCreate(
            HttpServletRequest _req, HttpServletResponse _resp, Writer aw) throws Exception {
    ...
    /**
     * This is the base URL for the application, which means it has the protocol, server, port, and
     * application name in the path. Everything up to the root of where the application is.
     */
    ...
    private void addIfNotNull(ArrayList<String> dest, String source, int start, int pos)
            throws Exception {
        if (pos <= start) {
            return;
        }
    ...
    public JSONObject getPostedObject() throws Exception {
        ...
        try {
            ...
            throw new Exception(
                    "Failure to read an expected JSON object from the POST stream for this web request: "
                            + requestURL,
                    e);
        }
    }
    ...
    /**
     * Reads the uploaded PUT body, and stores it to the specified file (using a temp name, and
     * deleting whatever file might have been there before.)
     */
    ...
    public static void streamException(
            Throwable e, HttpServletRequest request, HttpServletResponse response, Writer w) {
        try {
            if (w == null) {
                JSONException.traceException(
                        e, "a null writer object was passed into streamException");
            ...
            JSONObject responseBody =
                    JSONException.convertToJSON(e, "Web request for: " + request.getRequestURI());

            // remove the bottom of the stack trace below the HttpServlet.service call
            // because it is all arbitrary garbage below that point and usually quite a lot of
            // noise.
    ...
    public void streamFile(File fullPath) throws Exception {
        if (!fullPath.exists()) {
            throw new Exception(
                    "Program Logic Error: WebRequest.streamFile was asked to stream a file that does not exist: "
                            + fullPath);
        }
    ...
    /**
     * During the course of this session, if setAuthUserId has been called, then this will return
     * the value that was given at that time. You can implement your own login methods and this
     * simply records the result in the current session. Returns null if not authenticated.
     */
    ...
    /**
     * You can implement your own login methods and this simply records the result in the current
     * session. During the course of this session, if this has been called, then getAuthUserId will
     * return the value that was given at that time. Log a person OUT by setting this to null.
     */
    ...
    public String getConfigSetting(String name) throws Exception {
        SessionManager smgr =
                (SessionManager)
                        request.getSession()
                                .getServletContext()
                                .getAttribute("GlobalSessionManager");
