...
/**
 * The purpose of this is to be the root-most singleton server object that also manages Tenant-based
 * user session limits. It listens to the Servlet container for sessions being destroyed.
 *
 * <p>This is a singleton object.
 *
 * <p>It is also used as the object to get and maintain all the metadata about the server and
 * tenants. It will read the configuration, and provide this to the rest of the module.
 *
 * <p>This object will maintain a single super session with the server, and it will allow users'
 * access to that super session based on configuration settings. This will be used to gather the
 * list of all the tenants in the server, and the list of users for each tenant.
 */
public class SessionManager implements HttpSessionListener {
    ...
    private SessionManager(ServletContext _sc) {
        try {
            sc = _sc;
        } catch (Exception e) {
            throw new SimpleException(
                    "Unable to initialize the server, something is probably wrong with "
                            + "the configuration or the operating environment.",
                    e);
        }
    }

    public static SessionManager getSessionManagerSingleton(ServletContext sc) {
        File webInfPath = new File(sc.getRealPath("/WEB-INF"));
        if (!webInfPath.exists()) {
            System.out.println(
                    "ServerCore.getServerCoreSingleton: The servlet context claims the WEB-INF location, but not there: "
        ...
        if (smgr == null) {
            ...
            try {
                ...
                System.out.println(
                        "FAILURE installing session listener in SessionLimitManager.  Session limits can NOT be enforced! "
                                + e);
            }
        } else {
            // update the pointer to the servlet core object... and out of date one might have been
            // persisted.
            smgr.sc = sc;
        }
        return smgr;
    ...
    public File getAppDataFolder() {
        try {
            ...
            if (!webInfFolder.exists()) {
                throw new SimpleException(
                        "For some reason webInfFolder (%s) does not exist!",
                        webInfFolder.getAbsoluteFile());
            }
            ...
            throw new SimpleException("Unable to get the AppFolder", e);
        }
    }
    ...
    public JSONObject getConfigSettings() {
        ...
        if (!appConfigFile.exists()) {
            throw new SimpleException(
                    "Config File (%s) does not exist!", appConfigFile.getAbsoluteFile());
        }
