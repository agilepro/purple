...
public class WebClient {
    ...
    /** Send a JSONObject to this server as a POST and get a JSONObject back with the response. */
    ...
    /** Send a String to this server as a POST and get a JSONObject back with the response. */
    ...
    /** Send a JSONObject to this server as a PUT and get a JSONObject back with the response. */
    ...
    /** Send a Object to this server as a POST and get a JSONObject back with the response. */
    private JSONObject sendRequestToRemote(
            URL url, Object msg, String method, String auth, String contentType) throws Exception {
        ...
        try {
            ...
            throw new SimpleException(
                    "WebClient failed to send %s request to server url %s. The request and response is %s",
            ...
            if (is != null) {
                try {
                    ...
                    SimpleException.traceException(
                            ioe,
                            "WebClient encountered an error when closing input stream for url "
                                    + url);
                }
            }
            if (osw != null) {
                try {
                    ...
                    SimpleException.traceException(
                            ioe2,
                            "WebClient encountered an error when closing output stream for url "
                                    + url);
                }
            }
        }
    ...
    public JSONObject requestToRemote(URL url, JSONObject msg, String method, String auth)
            throws Exception {
        return sendRequestToRemote(url, msg, method, auth, "application/json");
    }

    /**
     * Performs a HEAD operation to test if something exists or not. Return true if the response
     * code is 200 False if 400 or anything else.
     */
    public boolean existTest(URL url) throws Exception {
        try {
            ...
            if (responseCode == 200) {
                ...
                throw new Exception(
                        "WebClient existTest for url "
                                + url
                                + " encountered a http error code "
                                + responseCode);
            }
        } catch (Exception e) {
            SimpleException.traceException(
                    e, "WebClient existTest for url " + url + " encountered an exception");
            throw new SimpleException(
                    "WebClient existTest for url (%s) encountered an exception", e, url);
        }
    }
}
