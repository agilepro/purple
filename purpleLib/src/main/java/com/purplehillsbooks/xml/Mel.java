 ...
 *
 * <p>The purpose of this class is to be a base class to classes which are designed to be interfaced
 * to DOM trees. If you have a DOM tree, and you want to directly reflect that out to the program as
 * structured data, you create a file that has getters and setters for the various members. This
 * base class containes convenient methods for manipulating the DOM tree, and keeping it in sync
 * with the external representation you are presenting.
 *
 * <p>GETTERS: pull values directly from child dom elements. Useful when a particular member is
 * gotten only a few times any reading of the file. If this is a heavily used member, prefectch that
 * to a cache member, and use that. SETTERS: put value directly into child dom elements. If you have
 * decided to use a cache member for fast access, don't forget to update both the cache and the dom
 * tree.
 *
...
public class Mel {
    ...
    /**
     * Standard constructor for a Mel on an existing XML tree. If you have a XML document which you
     * know is a MDS file you can use this constructor to make a single Mel that wraps an XML
     * element. Children elements are not created until asked for.
     */
    public Mel(Document doc, Element ele) {
        if (ele == null) {
            throw new RuntimeException(
                    "Program logic error: Mel object can not"
    ...
    /**
     * Constructs an instance of an extended class. This is used by all the methods that take a
    ...
    /**
     * This internal method is used to create a child of this object, of the specified class, once
     * the DOM element is found. The schema reference is passed on the child.
     */
    ...
    /**
     * Given a File object (that points to a real existing MDS file) This global static method will
     * read the file and return a Mel for the base of the tree of the file.
     */
    ...
    /**
     * Given a byte stream (that points to a real existing MDS file) this global static method will
     * read the stream and return a Mel for the base of the tree of the file. Encoding is always
     * UTF-8.
     */
    ...
    /**
     * You really should never use this. XML should only be handled as a stream of bytes and NEVER
     * as a string, however, sometimes particular poorly designed API will pass around XML as a
     * string, and this convenient method will parse it for you.
     */
    public static <T extends Mel> T parseString(String input, Class<T> rootClass) throws Exception {
    ...
    /**
     * writeToOutputStream streams the entire XML output that reflects the entire tree to an output
     * stream. Encoding is always UTF-8.
     */
    public void writeToOutputStream(OutputStream out) throws Exception {
        ...
        // see:
        // https://stackoverflow.com/questions/15592025/transformer-setoutputpropertyoutputkeys-encoding-utf-8-is-not-working/47683768#47683768
    ...
    /**
     * You really don't want to ever have XML in a string. XML should always be a byte stream and
     * only the values turned into Strings when needed, but there are times that you need the XML as
     * a string to pass to some other processing unit. This allows getting the XML formatted as a
     * string. Try not to ever use this.
     */
    ...
    /**
     * As you might have guessed from the name, the writeToFile method will write the entire tree
     * out to a file. You must pass in a valid full path to a file name.
     *
     * <p>To assure atomic writing, the file is first output to a temporary file that is the same
     * name, but with a "tmp-###" appended to the end of the name. Only when the entire tree is
     * successfully output to the file, then the file is renamed to the desired name.
     *
     * <p>If there was a file already there with that name, then that file will be deleted, just
     * before the temporary file is renamed to the desired name.
     *
     * <p>If your program crashes for any reason before the entire file is output, the result will
     * be that the previous version of the file will still be there, and there will be a partially
     * written file with a "tmp-###" appended to the end. Since these are most likely fragments of
     * the entire file, they are not valid XML files, and can not be parsed. They should be
     * discarded, and the cause of the crash researched. There are no normal situation that these
     * files should be seen left in the file system.
     *
     * <p>Here is a specific example. If you want to write to a file with the path
     * "c:/data/MyConfig.cfg" the folder "c:/data" must exist. The output routine will choose a
     * random number, for example '137' and will write the contents of the tree to
     * "c:/data/MyConfig.cfg-tmp-137". Once the file is completely output to disk, the old file
     * "MyConfig.cfg" is deleted, and the temp file is renamed to "MyConfig.cfg".
     *
     * <p>With this approach you are most likely to always have a valid file with the desired name.
     * There is an extremely small possibility that the program might crash between the deletion of
     * the old file, and the renaming of the temp file, leaving you with no file at all with the
     * desired name. There is a very small window of time between these two operations where another
     * program or another thread might find the file missing from the file system. This seems to be
     * a limitation that we have to live with until Java offer an atomic
     * rename-and-delete-previous-file operation.
     */
    ...
    /** This returns the namespace prefix, if there is one. Returns an empty string if not. */
    ...
    /**
     * Generally a Mel is a container, meaning it can have children which are either more Mel or
     * data values. When retrieving a data value (either scalar or vector), the methods return the
     * value directly. But some methods return all children as a single collection, including both
     * data values and containers. This is convenient for traversing the tree and generating output.
     * In those situations you need to call this method "isContainer" to determine if this
     * particular node contains only text, or whether it can have structured children.
     *
     * <p>Note: there is one situation where it is impossible to know whether it is a container or
     * not, and that is when the tag is completely empty and it has no attributes. In this case it
     * might be a data value without any data, or it might be a container without any contents.
     * Either way it is empty. This function returns false in this ambiguous case, thus an element
     * that is normally a container will return false (not a container) if it is completely empty.
     */
    ...
    /**
     * If you call "getAllChildren" you will get Mels that represent containers, as well as ones
     * that represent data. If the "isContainer" method returns false, then it represents a data
     * element, and this method will return the string value for that data element.
     */
    ...
    /**
     * If a containing tag has an attribute, getAttribute will return the string value of that
     * attribute.
     *
     * <p>attrName: the name of the attribute holding a data value returns: a string values
     */
    public String getAttribute(String attrName) {
        if (attrName == null) {
            throw new RuntimeException(
                    "Program logic error: a null attribute name" + " was passed to getAttribute.");
        }
        return fEle.getAttribute(attrName);
    }

    /**
     * If a containing tag has an attribute, and that attribute contains an integer string value,
     * then getAttribute will return the long value of that attribute.
     *
     * <p>A "forgiving" conversion is used which does not complain if non-numeral characters are
     * included, it simply uses any numerals it finds to return the result as if it was only
     * numerals. If there are no numerals at all, or if the attribute does not have a value, it
     * simply returns zero. The reason for using this is because MDF files are designed for
     * interchange between systems. If the value contains an integer then this conversion will
     * proceed correctly, but if the file generated by some other program has the wrong value in the
     * attribute, the problem is essentially ignored. This approach allows commas (or periods) to
     * group thousands and they will be ignored. It successfully ignores all currency symbols or
     * other unit indicators that might be there. Most importantly, it does not throw exceptions
     * ever.
     *
     * <p>attrName: the name of the attribute holding a data value returns: a long value
     */
    ...
    /**
     * Sets an attribute and value on a tag. This will replace any value that already exists there.
     *
     * <p>attrName: the name of the attribute holding a data value value: a string value
     */
    public void setAttribute(String attrName, String value) {
        if (attrName == null) {
            throw new RuntimeException(
                    "Program logic error: a null attribute name" + " was passed to setAttribute.");
        }
        if (value == null) {
    ...
    /**
     * Sets an attribute and an integer (long) value on a tag. This will replace any value that
     * already exists there.
     *
     * <p>attrName: the name of the attribute holding a data value value: a long value
     */
    ...
    /**
     * Retrieves an attribute value from a tag, and compares it to a given value, returning true if
     * they are equal. This is convenient when looking through a list of data elements to find one
     * with a particular attribute value. The most common use for this is an "id" attribute. This is
     * convenient for finding a particular data element with a particular id value.
     *
     * <p>attrName: the name of the attribute holding a data value testValue: a string value to test
     * against
     */
    ...
    /**
     * If a containing tag has a child value tag getScalar will find that tag, and return the string
     * value.
     *
     * <p>memberName: the name of the tag holding a data value returns: a string value
     */
    public String getScalar(String memberName) {
        if (memberName == null) {
            throw new RuntimeException(
                    "Program logic error: a null member name" + " was passed to getScalar.");
        }
        return getChildText(fEle, memberName);
    }

    /**
     * setScalar will create a child value tag of a specified name and value for the containing
     * element. setScalar will also remove the child value tag if you pass a null for the value
     * parameter.
     *
     * <p>memberName: the name of the tag holding a data value value: a string value to put in the
     * tag.
     */
    public void setScalar(String memberName, String value) {
        if (memberName == null) {
            throw new RuntimeException(
                    "Program logic error: a null member name" + " was passed to setScalar.");
        }
    ...
    /**
     * If a containing tag has multiple child value tags you can access all the values at once,
     * retrieving a vector of string values.
     *
     * <p>Call getVector with the name of the tag(s) that hold the data. Each such tag will be
     * accessed, and the value included in the vector in the same order.
     *
     * <p>memberName: the name of the tag holding a data value returns: a List of string values
     */
    public List<String> getVector(String memberName) {
        if (memberName == null) {
            throw new RuntimeException(
                    "Program logic error: a null member name" + " was passed to getVector.");
        }
    ...
    /**
     * If a containing tag has multiple child value tags you can access and set all the values at
     * once using a vector of string values.
     *
     * <p>Construct a vector will all of the string value in the correct order. Call setVector, and
     * a child tag and value will be created in the DOM tree for each value in the vector. Any
     * previous values will be removed.
     *
     * <p>memberName: the name of the tag holding a data value values: a List of string values
     */
    public void setVector(String memberName, List<String> values) {
        if (memberName == null) {
            throw new RuntimeException(
                    "Program logic error: a null member name" + " was passed to setVector.");
        }
    ...
    /**
     * If a containing tag has multiple child value tags you append a value to that set of values.
     *
     * <p>memberName: the name of the tag holding a data value value: a string value to be added
     */
    public void addVectorValue(String memberName, String value) {
        if (memberName == null) {
            throw new RuntimeException(
                    "Program logic error: a null member name" + " was passed to addVectorValue.");
        }
    ...
    /**
     * Creates a child element of the Mel class. Use this when you don't have a custom Mel class to
     * handle that part of the document.
     */
    ...
    /**
     * Returns a child Mel object with a specified name. If you pass an index of 0, it will return
    ...
    /**
     * Returns a child object of the specified class with a specified name. If you pass an index of
     * 0, it will return the first element found.
     */
    ...
    public <T extends Mel> T findChild(
            String elementName, String attributeName, String keyValue, Class<T> childClass)
            throws Exception {
        NodeList childNdList = fEle.getChildNodes();
        for (int i = 0; i < childNdList.getLength(); i++) {
            org.w3c.dom.Node n = childNdList.item(i);
    ...
    /**
     * getAllChildren will return a vector of Mels, one for each existing child element, as well as
     * one for each existing data value. This is a rare case that data is returned not as a string,
     * but as a Mel. You must use "isContainer" method to determine whether this element is a data
     * value or not.
     */
    ...
    /**
     * designed primarily for returning date long values works only for positive integer (long)
     * values considers all numeral, ignores all letter and punctuation never throws an exception if
     * you give this something that is not a number, you get surprising result. Zero if no numerals
     * at all.
     */
    ...
    private static Element createChildElement(
