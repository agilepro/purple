...
/**
 * The TestFrame framework provides a TestRecorder interface to each test upon invocation. The only
 * parameter to "TestSet.runTest" is a TestRecorder. Each Test Case uses this class to
 *
 * <ol>
 ...
 *
 * <p>For example, one implementation may read configuration information from a flat text file of
 * name-value pairs, while another may read it from an XML file Also, one implementation may write
 * the test results to a text file, while another may tie into a package like log4j to produce the
 * results.
 *
 * <p>All communications to the outside world that is not part of the test must be through the test
 * recorder.
 *
 * <p>Standard.out must NOT be written to, because there is no guarantee that output will be
 * accessible. You must not open your own log file for recording results. Calling markPassed or
 * markFailed will assure that the result of the test is properly recorded, and entered in all
 * statistics.
 *
 * <p>There is a capability to produce "debug" output while you are developing the test, see the
 * "log" method.
 *
 * <p>TestRecorder implementations should have "verbose" as one of the configuration parameters.
 * When verbose=false (normal situations) output should only be produced for failed and fatal tests.
 * The logic is that normally we will have hundreds of tests, and they all pass so we only need to
 * know about tests which failed or had fatal errors. When verbose=true, output should be produced
 * for passed tests and log information (recorded using the log method), in addition to the output
 * produced when verbose=false. This allows you to get a listing of all the tests that were run if
 * you wish, along with any "debug" information that the tests may have put out.
 *
 * @see TestSet
 * @see TestDriver
...
public interface TestRecorder {
    ...
    /**
     * Returns the value of the 'verbose' configuration parameter. Tests will ususally not need to
     * call this method.
     */
    public boolean isVerbose();

    /**
     * A configuration file 'test.conf' is read automatically and is represented in the TestRecorder
     * as a Properties object. Standard global configuration parameters should be placed in
     * 'test.conf' and the tests can get access to those configuration parameters using this method.
     * If the property does not exist, then default is returned.
     */
     ...
     * as a Properties object. Standard global configuration parameters should be placed in
     * 'test.conf' and the tests can get access to those configuration parameters using this method.
     * The specified property MUST exist in the properties file. Throws a nice exception if the
     * property is missing. Use this for properties like "serverName" where there is no way to
     * provide a reaonsable default. If you need a particular configuration value, then if it is not
     * configured, then this is a configuration problem that must be addressed before running the
     * tests. Throwing an exception will cause termination of the testing due to a fatal error, and
     * it will display the reason for the fatal, which will encourage the tester to set the
     * configuration up correctly.
     */
    public String getRequiredProperty(String name) throws Exception;

    /**
     * getTestDir returns the path to the folder that contains all the test files. If a test needs
     * to read such a file, get this path and append the name of the file on to it.
     *
     * <p>You should assume this directory is "read only"
     */
    ...
    /**
     * Tests generally should not use this method but rather should use the convenient routines
     * getProperty and getRequiredProperty,
     */
    public Properties getProps();

    /**
     * setProps should not be used except in extreme situations where particular properties need to
     * be added or modified for a particular test.
     */
    public void setProps(Properties props);

    /**
     * The command line arguments to the main routine are stored in the TestRecorder. Individual
     * tests can have access to them using this method, and can use them for any purpose necessary.
     * If the command line pointed to a text file with a list of tests, each line of that file
     * contains the name of a class file, along with parameters, and those parameters will appear in
     * the returned string array.
     */
    public String[] getCommandLineArgs();

    /**
     * The TestDriver may call this method if it needs to add/modify the command line arguments.
     * Tests should not call this method.
     */
