...
/**
 * Given a file folder, when asked for a schema XYZ this class will read a file 'Schema-XYZ.json'
 * and return the JSONObject to the caller.
 *
 * <p>A cache of schema objects are kept in memory for reuse as needed until you discard the
 * SchemaLibrary.
 */
        ...
        try {
            if (!folder.exists()) {
                throw new SimpleException(
                        "FileSchemaLibrary did not find a file folder at (%s)",
            ...
            if (!expectedFile.exists()) {
                throw new SimpleException(
                        "Schema file does not exist: %s", expectedFile.getAbsolutePath());
            }
