 ...
 * </pre>
 *
 * <p>Basically, if an object member is the same between the two, then that member is omitted. If a
 * child object is exactly the same, it will be omitted. Only the members that are changed will be
 * included in the delta. If a new member is present then it is included. If a member value goes
 * away, then it is represented as a null string (""). If you have a list of simple values, and the
 * list changes, then the entire new list is included in the delta.
 *
 * <p>There is special handling for lists of objects. The list must have a key field identified, and
 * the objects in the list are compared by key. If the object (and all children of that object) are
 * the same, then it is omitted from the output. If a new object appears with a new key value, then
 * that new object is included so that it can be created along with all children. To delete an
 * object from a list, a special value is used that you can set, but it is @delete by default, and
 * this causes the entire object to be deleted on the receiving side.
 *
 * <p>The output would then be as follows: the title and page num tot remained the same, so they
 * will be omitted. The weight changed, so the new weight will be included. For the list, there must
 * be a key field, and according to the key if the object with the same key exists, and it is
 * unchanged, then that object is omitted. If there is a new object with a new key the entire object
 * will be included. If there is an existing object
 *
 * <pre>
 * {
 *     "weight": 2.22,
...
public class JSONDelta {
    ...
    /**
     * Creates a JSONObject that represents the delta of the two JSON objects passed in. Each member
     * that is the same is omitted, and only the members that are new or changed in the second
    ...
    private void compareInteger(JSONObject result, String key, Object oldValue, Object newValue)
            throws Exception {
    ...
    private void compareString(JSONObject result, String key, Object oldValue, Object newValue)
            throws Exception {
    ...
    private void compareDouble(JSONObject result, String key, Object oldValue, Object newValue)
            throws Exception {
    ...
    private void compareLong(JSONObject result, String key, Object oldValue, Object newValue)
            throws Exception {
    ...
    private void compareBoolean(JSONObject result, String key, Object oldValue, Object newValue)
            throws Exception {
    ...
    private void compareJSONObject(JSONObject result, String key, Object oldValue, Object newValue)
            throws Exception {
    ...
    private void compareJSONArray(JSONObject result, String key, Object oldValue, Object newValue)
            throws Exception {
    ...
    private JSONObject findKeyedObject(JSONArray source, String keyMember, String idValue)
            throws Exception {
    ...
    /**
     * Hhere we have determined that either there are non-objects in the list or those objects don't
     * have an id value, so we can't compare them by id for insertions and deletions.
     *
     * <p>So this compares for EXACT equal lists. If nothing has changed in a list then it can be
     * omitted completely. If there is any difference then the entire list must be set.
     *
     * <p>As soon as any single difference is found in the list of items, just set the new as a
     * value and be done with it
     */
    private void compareExactJSONArray(
            JSONObject result, String key, JSONArray oldList, JSONArray newList) throws Exception {
     ...
     * optional parameter (-a) which controls the reportAll setting.
     *
     * <p>The output will be written to second file name with "delta.json" on the end. In the
     * example above, the file would be written to <tt>Second-File.JSONDelta.csv</tt>
     */
