...
/**
 * The JSONSChema class has a static method for creating a schema object, and can be instantiated to
...
public class JSONSchema {
    ...
    /**
     * during validation, errors will be collected up to this limit, and the validation will return
     * when the limit is reached. If you just want to find the first error, set the limit to 1.
     */
    public int errorLimit = 100;

    /**
     * if this is set false, then success (correct) validation is silent if set to true, then every
    ...
    /**
     * If the schema has a #ref token, you will need to supply a SchemaLibrary that will retrieve
     * the reference for the schema that is named.
     *
     * <p>Any attempt to validate a schema which inludes a #ref will fail if there is no schema
     * library to supply the references schema.
     */
    ...
    /**
     * Generates a schema object from the given instance. This is a default schema which will
     * probably have to be edited to take into consideration variances. OF course, it will only
     * include data members that exist. It assumes that all members are required. It uses the type
     * of value found in the first instance of a member to set the type of that members. If there is
     * an array, it walks through the array and finds the union of all the members on the objects.
     * If you want to allow additional members you need to create your own additionalProperties
     * entry. If you want to allow members to have varying types, you will need to remove the type
     * indicator.
     */
    public static JSONObject generateSchema(JSONObject data) throws Exception {
        ...
        if (requiredList.length() > 0) {
            // OpenAPI Swagger schema documentation specifically says an empty array is not allowed
            // for required.
    ...
    /**
     * This walks through the array looking at each member, getting the schema for each member, and
     * merging the results together. If the objects are the same, then this should have the effect
     * of collecting all of the optional elements that exist in the data. If objects are different,
     * then this might produce a mess, but that would invalidate the idea of schema in the first
     * place.
     *
     * <p>If there are differences in the array items, then the LAST item will take the most
     * precedence.
     */
    ...
    private boolean checkSchemaRunner(String path, JSONObject data, JSONObject schema)
            throws Exception {
        try {
            ...
            if (!schema.has("type")) {
                ...
                addLog(
                        "Found Object at '"
                                + path
                                + "' but skipping check becaue schema does not specify type");
                return true;
            }
            String mainType = schema.getString("type");
            if (!"object".equals(mainType)) {
                addErrorLog(
                        "@" + path + " - found an object but schema is expecting a " + mainType);
                return false;
            }
            ...
            for (String key : data.keySet()) {
                ...
                if (d instanceof JSONArray) {
                    ...
                    if (!"string".equals(dataType)) {
                        addErrorLog(
                                "@"
                                        + path
                                        + key
                                        + ". - found a string but expecting a "
                                        + dataType);
                        resultCode = false;
                    } else {
                    ...
                    if ("integer".equals(dataType)) {
                        ...
                        addErrorLog(
                                "@"
                                        + path
                                        + key
                                        + ". - found an integer but expecting a "
                                        + dataType);
                        resultCode = false;
                    }
                    ...
                    if ("integer".equals(dataType)) {
                        ...
                        addErrorLog(
                                "@" + path + key + ". - found a long but expecting a " + dataType);
                        resultCode = false;
                    }
                        ...
                        if (f == f.intValue()) {
                            ...
                            addErrorLog(
                                    "@"
                                            + path
                                            + key
                                            + ". - found a non-integer number but expecting an integer");
                            resultCode = false;
                        }
                    } else if ("long".equals(dataType)) {
                        Double f = (Double) d;
                        if (f == f.intValue()) {
                            ...
                            addErrorLog(
                                    "@"
                                            + path
                                            + key
                                            + ". - found a non-integer number but expecting a long");
                            resultCode = false;
                        }
                    } else {
                        addErrorLog(
                                "@"
                                        + path
                                        + key
                                        + ". - found a number but expecting a "
                                        + dataType);
                        resultCode = false;
                    }
                    ...
                    if (!"boolean".equals(dataType)) {
                        addErrorLog(
                                "@"
                                        + path
                                        + key
                                        + ". - found a boolean but expecting a "
                                        + dataType);
                        resultCode = false;
                    } else {
                    ...
                    if (!data.has(requiredPropertyName)) {
                        addErrorLog(
                                "@"
                                        ...
                                        + " property required by schema not found");
                        resultCode = false;
                    }
                }
            }
    ...
    private boolean checkSchemaArray(String path, JSONArray data, JSONObject schema)
            throws Exception {
        try {
            if (errors.size() > this.errorLimit) {
                return false;
            ...
            if (!"array".equals(schema.getString("type"))) {
                addErrorLog(
                        "@"
                                + path
                                + " - found an array, but schema expects it to be: "
                                + schema.getString("type"));
                return false;
            }
            if (data.length() == 0) {
            ...
            if (!schema.has("items")) {
                // missing items is treated like a missing type:  no validation of the elements will
                // be done.
                return true;
            }
                    ...
                    if (!"integer".equals(dataType) && !"number".equals(dataType)) {
                        addErrorLog(
                                "@"
                                        ...
                                        + "["
                                        + i
                                        + "]. - found a integer but expecting a "
                                        + dataType);
                        resultCode = false;
                    } else {
    ...
    /**
     * if the schema validation returns false, then you can ask for a detailed list of what went
     * wrong with this method.
     *
     * <p>Each string in the list is a separate validation error.
     *
     * <p>You will only get up to the number of errors specified in the errorLimit variable.
     */
