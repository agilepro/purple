...
/**
 * This is a TemplateStreamer that gets all the information for the tokens from JSON object tree.
 * The token values are expressed as path expressions with dots between them. For example, for a
 ...
 * </pre>
 *
 * <p>Expressions that accurately address leaf nodes will write that string value If the leaf is a
 * number it will write a string representation of that. If the expression ends at an object or an
 * array, it will write nothing. If the expression addresses a member that does not exist, it writes
 * nothing.
 */
public class TemplateJSONRetriever implements TemplateTokenRetriever {
    ...
    public void writeTokenDate(Writer out, String token, String format) throws Exception {
        try {
            ...
            throw new Exception(
                    "Unable to write Date value from path " + token + ", with format " + format, e);
        }
    }
    ...
    private Object getValueFromContext(String token) throws Exception {
        ...
        if (itArray != null) {
            ...
            if (o == null) {
                throw new Exception(
                        "Problem that loop have been initiated, but the setIteration has not been called");
            }
            if (tokens.size() <= 1) {
                val = o;
    ...
    public int initLoop(String id, String token) throws Exception {
        ...
        if (o instanceof JSONArray) {
            ...
            throw new Exception(
                    "Loop data path parameter must address a JSON Array, but this one does not: "
                            + token);
        }
    }
    ...
    public void setIteration(String id, int loopCount) throws Exception {
        ...
        if (itArray == null) {
            throw new Exception(
                    "Loop problem, no loop with id (" + id + ") appears to have been initialized.");
        }
        if (loopCount >= itArray.length()) {
            throw new Exception(
                    "Loop problem, seem to have iterated off end of array: "
                            + loopCount
                            + " is greater than "
                            + (itArray.length() - 1));
        }
    ...
    private static Object getValuefromObject(ArrayList<String> tokens, int index, JSONObject d)
            throws Exception {
    ...
    private static Object getValuefromArray(ArrayList<String> tokens, int index, JSONArray ja)
            throws Exception {
        String thisToken = tokens.get(index);
    ...
    /**
     * Breaks a string into a list of strings using dots (periods) as separators of the token, and
     * trimming each token of spaces if there are any.
     */
